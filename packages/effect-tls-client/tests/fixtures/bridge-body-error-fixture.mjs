#!/usr/bin/env node

import packageJson from "../../package.json" with { type: "json" };

let buffer = Buffer.alloc(0);
const writeFrame = (kind, id, metadata = {}, body) => {
  const meta = Buffer.from(JSON.stringify(metadata));
  const payload = body ?? Buffer.alloc(0);
  const rest = Buffer.alloc(9 + meta.length + payload.length);
  rest.writeUInt8(kind, 0);
  rest.writeUInt32BE(id, 1);
  rest.writeUInt32BE(meta.length, 5);
  meta.copy(rest, 9);
  payload.copy(rest, 9 + meta.length);
  const frame = Buffer.alloc(4 + rest.length);
  frame.writeUInt32BE(rest.length, 0);
  rest.copy(frame, 4);
  process.stdout.write(frame);
};

process.stdin.on("data", (chunk) => {
  buffer = Buffer.concat([buffer, chunk]);
  while (buffer.length >= 4) {
    const length = buffer.readUInt32BE(0);
    if (buffer.length < length + 4) return;
    const frame = buffer.subarray(4, length + 4);
    buffer = buffer.subarray(length + 4);
    const kind = frame.readUInt8(0);
    const id = frame.readUInt32BE(1);
    const metadataLength = frame.readUInt32BE(5);
    const metadata = JSON.parse(
      frame.subarray(9, 9 + metadataLength).toString("utf8"),
    );
    if (kind === 0x01) {
      writeFrame(0x80, 0, {
        protocolVersion: 1,
        bridgeVersion: packageJson.version,
        tlsClientVersion: "fixture",
        goVersion: "fixture-go",
      });
    } else if (kind === 0x10) {
      writeFrame(0x81, id, {});
    } else if (kind === 0x11) {
      writeFrame(0x81, id, {});
    } else if (kind === 0x20) {
      if (metadata.hasBody === true) continue;
      writeFrame(0x82, id, {
        kind: "Internal",
        message: "body test requires an upload",
      });
    } else if (kind === 0x40) {
      writeFrame(0x82, id, {
        kind: "Cancelled",
        message: "operation cancelled",
      });
    } else if (kind === 0x02) {
      writeFrame(0x81, 0, {});
      process.exit(0);
    }
  }
});
